...
- The dialogue FOV zoom is off (no separate "No Dialogue Zoom" mod needed).
...
**Compatibility:** this mod overrides `AnimBP_Player`, `BP_Stalker2Character` and `IMC_Dialog` and is named to load after other mods. Any mod that overrides the same assets (some weapon-positioning mods ship `AnimBP_Player`) will lose its changes while this is installed. Each game patch needs a rebuild; watch the Nexus page.
