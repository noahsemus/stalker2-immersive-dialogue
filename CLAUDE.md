...
Current shipped state: **v1.0.4** on Nexus (<https://www.nexusmods.com/stalker2heartofchornobyl/mods/2698>) and GitHub releases. First Nexus release (v1.0) cut on 2026-09-11; the gesture-camera fix ("v1.1" work in earlier commits) is folded into that v1.0 tag. v1.0.1 (2026-09-12): `IsUnreachable()` check on the LookAt modifier loop. v1.0.2 (2026-09-12): the `Resolve*` cache calls in `on_update` are gated on `inDlg` — they used to re-run one tick after every dialogue exit and cache the current world's camera manager / mesh / anim instances, which a subsequent save load destroyed; `IsUnreachable()` doesn't catch freed slots, so the next dialogue crashed in `RemoveCameraModifier`. v1.0.3 (2026-09-12 evening): the dialogue-entry freeze — three full `ForEachUObject` scans + a property dump ran inside the first in-dialogue tick (1.44 s of stopped game thread, read by users as "no control during the zoom"); BH/dummy anim instances now come from the mesh's `LinkedInstances`, LookAt modifiers from the camera manager's `ModifierList`, footstep/anim probes deleted. Also: config.ini resolved by our own module address + created on first launch; `[PC+0x650]` null guard on the `IsInStaticDialog` poll (crash on quit); move vector zeroed on dialogue exit; input paused while the mouse cursor is shown in dialogue (trade/inventory screens).

v1.0.4 (2026-09-13): native Sony pad support. A DualSense is not an XInput device; `XInputGetState` only sees it while Steam Input (or DS4Windows) translates it into a virtual Xbox pad, so players with Steam Input off (common — it's how you keep the game's native DualSense haptics) reported "works on keyboard, not on controller" (Nexus, Saigaiii866, 2026-09-13). Fix: raw-input registration for HID gamepads (usage page 1 / usage 5), Sony VID 0x054C report parsing in the WndProc hook, XInput still preferred when present. Never re-registers a usage the game already registered (would redirect its WM_INPUT stream); subclasses that window read-only instead. The log line `[ImmDlg] pad source: xinput|sony-hid|none` says which path is live. Verified in-game by Noah with Steam Input off (log: `pad source: sony-hid`).
