...
1. Noah confirms in-game. 2. Mirror assets to repo, commit on `main`, tag `vX.Y.Z`. 3. Zip the three `_20_P` files + README.txt as `ImmersiveDialogue-vX.Y.Z.zip` into `zonekit/builds/vX.Y.Z/`; same for the NoSkipHint plugin as `ImmersiveDialogue-NoSkipHint-vX.Y.Z.zip` (cook it with `-Mod ImmersiveDialogueNoSkipHint`, files `zzz_ImmersiveDialogueNoSkipHint_20_P.*`). 4. Push, `gh release create` with the zip and plain-language notes. 5. Remove the dev test pak folders `~mods\zzz_ImmersiveDialogue*_PakTest` (`revert_paktest.ps1`; Noah plays on the Vortex copy); copy the zips to `C:\Users\noahs\Downloads\` for the Nexus upload (mod 2698; NoSkipHint goes up as an optional file).
