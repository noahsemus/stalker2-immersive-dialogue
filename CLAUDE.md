...
- **Planned: zero overrides** (zonekit/README.md § "Plan: universal compatibility", 2026-09-21): S-Watch (Nexus 2780) is the second watch mod colliding on `AnimBP_Player`. Chosen route: a mod-only post-process anim BP applied at spawn with `SetOverridePostProcessAnimBP` from the ModKit subsystem (walk pose layered over the vanilla output, gestures kept via the montage gate), then the 2.1 subsystem/own-IMC steps, then a runtime hide for the skip hint. Anim-class swap stays dead (crash); leader pose rejected (freezes gestures). Build A = pass-through graph, safety only.
