...
- Walk-start delay / A↔D flip stop: FIXED on branch `anim-2.0.2` (unreleased): the anim BP now derives DlgMoving/DlgFwd/DlgRight from the pawn's `MovementInputVector` (what `SetMoveVector` writes) with a 0.15 s hold, not from velocity (velocity dips through zero on a reversal → StopWalk → restart). Checkpoint pak `zonekit/builds/checkpoint-2026-09-14-inputdrive/`.
- Open: Skif's arms are not visible in dialogue except while a gesture montage plays (look down while walking: no arms). Must not disturb gestures (IsAnyMontagePlaying → straight legs).
...
1. Noah confirms in-game. 2. Mirror assets to repo, commit on `main`, tag `vX.Y.Z`. 3. Zip the three `_20_P` files + README.txt as `ImmersiveDialogue-vX.Y.Z.zip` into `zonekit/builds/vX.Y.Z/`. 4. Push, `gh release create` with the zip and plain-language notes. 5. Remove the dev test pak folder `~mods\zzz_ImmersiveDialogue_PakTest` (Noah plays on the Vortex copy); copy the zip to `C:\Users\noahs\Downloads\` for the Nexus upload (mod 2698).
