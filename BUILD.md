...
A Zone Kit "plain mod" plugin named `ImmersiveDialogue`. From 2.1.0 it
**overrides two game assets** (the player pawn and `IMC_Dialog`) by placing
edited copies at the same relative path under the mod's `Content` folder, and
ships **mod-only content** (a world subsystem and two animation Blueprints under
`/ImmersiveDialogue/Runtime/`) that animates the body in dialogue without
touching `AnimBP_Player` (§5.7). Up to 2.0.4 it also overrode `AnimBP_Player`
(§5.4, kept for reference). The kit cooks them into an "OverrideContent" pak; at runtime
...
| `Runtime/BP_ImmDlgSubsystem.uasset` (NewContent) | `ModWorldSubsystem` | attaches the dialogue body layer to the player mesh once per world, enables it only in dialogue (§5.7) |
| `Runtime/ABP_ImmDlgBody.uasset` (NewContent) | anim Blueprint, post-process instance | copy of the 2.0.4 player anim BP, fed the main instance's data; the body in dialogue (§5.7) |
| `Runtime/ABP_ImmDlgHands.uasset` (NewContent) | anim Blueprint, linked layer | copy of `AnimBP_player_bh` without the forced hands-look switch; arms in dialogue (§5.7) |
| `Autogenerated_1789672966_WorldSubsystemData.uasset` (override list) | ModKit data | registers the subsystem with the game |
| ~~`_STALKER2/Animations/Player/AnimBP_Player.uasset`~~ | player animation Blueprint | up to 2.0.4 only; now in `zonekit/experiments/animbp-override-2.0.4/` |
...
**Dialogue input by game path (2.1.0).** Comment box `Dialog IMC: game path`
off the end of `Cam: init` (BeginPlay): `Make Soft Object Path`
("/Game/_Stalker_2/data/input/InputMappingContexts/IMC_Dialog.IMC_Dialog") ->
`To Soft Object Reference` -> `Load Asset Blocking` -> `Cast To InputMappingContext`
-> `SET DialogIMC`. `Add`, `Has` and `Remove Mapping Context` take `DialogIMC`
...
### 5.2b IMC_Dialog rows carry the game's mappable names (2.1.0)

The four keyboard move rows (W / A / S / D) have `SettingBehavior =
OverrideSettings` and a `PlayerMappableKeySettings` named like the exploration
rows (`MoveForward`, `MoveBack`, `MoveLeft`, `MoveRight`), plus four `*Alt` rows
with key `None` for the secondary slots. The game's Controls menu then writes a
`Dialogue` section for them in `%LOCALAPPDATA%\Stalker2\Saved\CustomizeControls.cfg`
and applies the player's keys to the **game-path** `IMC_Dialog` object at load.
Applied in place in the live editor with `zonekit/tools/add_mappable_to_imc.py`
(through `ue_exec.py`; editor remote execution is on). `make_imc_override.py`
does the same when rebuilding from scratch, but only in the commandlet
(`duplicate_asset` returns None in a running editor).

**Two copies of `IMC_Dialog` exist at runtime:** the override mounted at the game
path (`/Game/...`, the one the game rebinds) and the same package under the mod
path (`/ImmersiveDialogue/...`, never rebound). The Zone Kit editor redirects
every `/Game` pick of an overridden asset to the mod path, so the pawn loads the
game-path object by path string instead (§5.3, `Dialog IMC: game path`).
...
### 5.7 Dialogue body layer (2.1.0): no `AnimBP_Player` override

Engineering detail and the dead ends are in `zonekit/README.md` § "No
`AnimBP_Player` override". Summary of the assets:

**`Runtime/BP_ImmDlgSubsystem`** (`ModWorldSubsystem`, `OnTick`). Boxes:
- `Tick: split` — `Cast To PC` → Sequence `seq: tick`.
- `First sight` — `PawnSeenTime` set once.
- `Attach: soft` — if the mesh's post-process instance is not `ABP_ImmDlgBody`:
  `Set Disable Post Process Blueprint(false)`, `Set Override Post Process Anim BP
  (ABP_ImmDlgBody, Reinit false)`.
- `Attach: mesh swap once` — once per world, 1 s after the pawn appears and not
  in dialogue: save hidden bones (`Hidden bones: save`), override None, mesh =
  `SK_pla_sta_03_SHA`, override = `ABP_ImmDlgBody`, mesh back,
  `ToggleFOVAndForegroundRender(true)`, re-hide bones (`Hidden bones: restore`).
  A mesh change is the only safe Blueprint path that spawns the post-process
  instance; **never** call `Set Override Post Process Anim BP` with Reinit ticked
...
`AnimInstancePlayer`):
- Event graph box `Copy game data`: `seq: copy first` Then 0 → cast the mesh's
  main anim instance to `AnimInstancePlayer` ("main anim") → Set 22 data structs
  on self from it (everything BlueprintReadWrite except `DialogData`). Then 1 →
  the frozen 2.0.4 walk chain.
- `Link game layers` box (`LinkAnimClassLayers` on init): inert, kept.
- AnimGraph: the `WeaponLayer` linked layer node ("weapon layer: dialogue hands")
  has Instance Class `ABP_ImmDlgHands`. `Arms in dialogue` (idle graft) and
  "gesture from game" are at weight 0 (inert; gestures come through the hands
  layer, which reads the main instance's montage data).
...
subsystem data asset, `IMC_Dialog`, `BP_Stalker2Character`, the compat marker.
Ship both containers: the override one renamed `zzz_ImmersiveDialogue_20_P.*`,
the NewContent one under the kit's own name.
